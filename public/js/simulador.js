let simMap, simDirectionsService, simDirectionsRenderer, simOrigemAutocomplete, simDestinoAutocomplete;
...
function initSimMap() {
  ...
  // Autocomplete de moradas nos campos de origem e destino
  const origemInput = document.getElementById('sim-origem');
  const destinoInput = document.getElementById('sim-destino');
  const options = { fields: ['formatted_address', 'geometry'] };

  simOrigemAutocomplete = new google.maps.places.Autocomplete(origemInput, options);
  simDestinoAutocomplete = new google.maps.places.Autocomplete(destinoInput, options);
}
