...
router.get('/registo', (req, res) => {
  ...
  res.render('auth/registo', { title: 'Criar conta | SR Ride', erro: null });
});
...
router.post('/registo', async (req, res) => {
  ...
  try {
    ...
    res.render('auth/registo', { title: 'Criar conta | SR Ride', erro: 'Erro inesperado. Tenta novamente.' });
  }
});

router.get('/login', (req, res) => {
  if (req.session.user) return res.redirect('/conta');
  res.render('auth/login', { title: 'Entrar | SR Ride', erro: null, next: req.query.next || '/conta' });
});
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    const user = result.rows[0];
    if (!user) {
      return res.render('auth/login', { title: 'Entrar | SR Ride', erro: 'Credenciais inválidas.', next });
    }
    ...
    res.render('auth/login', { title: 'Entrar | SR Ride', erro: 'Erro inesperado. Tenta novamente.', next });
  }
});
...
router.get('/esqueci-password', (req, res) => {
  res.render('auth/esqueci-password', { title: 'Recuperar password | SR Ride', erro: null, sucesso: null });
});
...
router.post('/esqueci-password', async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      sendEmail({
        to: user.email,
        subject: 'Redefinir a tua password — SR Ride',
        html: brandedEmailTemplate({
          title: 'Redefinir password',
          bodyHtml: `
            <p style="margin:0 0 12px;">Olá ${user.nome},</p>
            <p style="margin:0 0 20px;">Recebemos um pedido para redefinir a password da tua conta SR Ride. Clica no botão abaixo para escolher uma nova password. Este link expira dentro de 1 hora.</p>
    ...
    res.render('auth/esqueci-password', { title: 'Recuperar password | SR Ride', erro: null, sucesso: genericSuccess });
  } catch (err) {
    console.error('Erro em esqueci-password:', err);
    res.render('auth/esqueci-password', { title: 'Recuperar password | SR Ride', erro: 'Erro inesperado. Tenta novamente.', sucesso: null });
  }
});
...
router.get('/redefinir-password/:token', async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) {
      return res.render('auth/redefinir-password', { title: 'Link inválido | SR Ride', token: null, erro: 'Este link é inválido ou já expirou. Pede um novo.' });
    }
    res.render('auth/redefinir-password', { title: 'Nova password | SR Ride', token: req.params.token, erro: null });
  } catch (err) {
    console.error('Erro ao verificar token:', err);
    res.render('auth/redefinir-password', { title: 'Erro | SR Ride', token: null, erro: 'Erro inesperado. Tenta novamente.' });
  }
});
...
router.post('/redefinir-password/:token', async (req, res) => {
  ...
  try {
    if (!password || password.length < 6) {
      return res.render('auth/redefinir-password', { title: 'Nova password | SR Ride', token, erro: 'A password precisa de pelo menos 6 caracteres.' });
    }
    const result = await pool.query(
      'SELECT id FROM users WHERE reset_token = $1 AND reset_token_expires > NOW()',
      [token]
    ...
    res.render('auth/redefinir-password', { title: 'Erro | SR Ride', token, erro: 'Erro inesperado. Tenta novamente.' });
  }
});
